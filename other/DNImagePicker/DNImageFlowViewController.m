...
- (void)setupView
{
    ...
    [self createBarButtonItemAtPosition:DNImagePickerNavigationBarPositionRight
                                   text:NSLocalizedStringFromTable(@"cancel", @"DNImagePicker", NSLocalizedString(@"cancel", nil))
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    NSInteger selectPhotoNum = self.kDNImageFlowMaxSeletedNumber==0? 9 : self.kDNImageFlowMaxSeletedNumber;
    
    if (self.selectedAssetsArray.count >= selectPhotoNum ) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"alertTitle", @"DNImagePicker", nil) message:[NSString stringWithFormat:@"不能超过%ld张图片",(long)selectPhotoNum] delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"alertButton", @"DNImagePicker", nil) otherButtonTitles:nil, nil];
        [alert show];
...
- (UICollectionView *)imageFlowCollectionView
{
    if (nil == _imageFlowCollectionView) {
        ...
        _imageFlowCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, [UIScreen mainScreen].bounds.size.height-150) collectionViewLayout:layout];
